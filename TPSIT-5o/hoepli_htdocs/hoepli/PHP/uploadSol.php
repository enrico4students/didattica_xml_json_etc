...
//fa vedere se presente un messaggio di errore
...
echo'
   <form action="uploadSol.php" method="post">
     <table>
	   <tr>
	     <td>
		   nome
...
elseif ( $_GET["query"]=="login")//verifica se il nome inserito è presente dentro il file
{
  ...
  if ($_GET["nome"]==$var2[$i])           // se coincidono
  {
	...
	$trovato=true;
	header("location:uploadSol.php"); // riaggiorno la pag
   }
  ...
  if  ($trovato==false)// il nome inserito nn è stato trovato nel file
   {
    echo "TU non sei registrato!!..<a href='uploadSol.php?query=iscrizione'>registrati!</a> oppure metti la psw giusta <a href='uploadSol.php?query=accedi'>accedi</a>";
   }
  fclose($var);
}
}
...
if ( $upload == "none" ) {
header("Location:uploadSol.php?msg=Non è stato inviato alcun file");
exit;
}
# lo copia in una nuova posizione
if (copy($tmp,"programmi/".$nome))
{
header("Location:uploadSol.php?msg=Caricamento file riuscito");
# cancella il file temporaneo
unlink($tmp);
} else {
header("Location:uploadSol.php?msg=Caricamento file fallito");
}
...
echo "<br><br><a href='uploadSol.php'>torna</a>";
$lista->close();
} 
}
if (isset($_POST["reg"]) ){               // se viene confermata la domanda d'iscriz...
	...
	header("Location:uploadSol.php?msg=Ti sei connesso ".$_POST["nome"]);
}
...
echo'
	 ...
	 <tr>
	     <td>
		   ...
		   <form name="foto" action="uploadSol.php" method="post" enctype="multipart/form-data">
...
echo'
	 ...
	 <tr>
	     <td>
		   <a href="uploadSol.php?query=accedi">accedi</a> oppure <a href="uploadSol.php?query=iscrizione">registrati</a> per caricare sul server un programma scolastico di una materia
		 <td>
	   </tr> 
	 </table>
';
