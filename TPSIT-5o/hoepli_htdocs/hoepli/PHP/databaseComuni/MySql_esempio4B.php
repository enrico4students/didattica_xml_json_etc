...
<?php
  ...
  if(isset($_POST['sub'])){
    ...
    for($i = 0; $i < count($comune); $i++){
      if ($result = $conn->query("SELECT codice_provincia_istat FROM comuni WHERE name ='".$comune[$i]."'"))
      {
      ...
      for($k = 0; $k < count($arrLink); $k++)
      {
        if($arrLink[$k] == $i)
        {
          if ($n > 0)
            echo ", ";
          $n++;
          //sanifica la parole - ma non funziona con le accentate !
          $cerca       = array("à","è","é","ì","ò","ù","'"," ","?");  
          $sostituisci = array("a","e","e","i","o","u","_","_","");
          $comune[$k] = str_replace($cerca, $sostituisci, $comune[$k]);
          echo $comune[$k];
        }
      }
      echo "<br>";
  ...
  {
    if ($result = $conn->query("SELECT name FROM comuni "))
    //if ($result = $conn->query("SELECT name FROM comuni ORDER BY name"))
    {
      echo "<form action='esempio4B.php' method='POST'>
